...
const Referral: React.FC<Props> = ({ getReferrals,createReferral,setNotification,configuration }) => {
		...
		if(valid){
			createReferral(data,(res) => {
				if(!res.errors){
					...
					setNotification({display:true,error:false,content:'A referral email has been sent to '+data.email,timed:true});
	...
	return (<section className={s.referrals}>
        <h2>Referrals</h2>
        <form id="referralForm" className={s.form} autoComplete="off" onSubmit={submit} noValidate>
            <fieldset>
                <label htmlFor="referral_name">Name</label>
                <input id="referral_name" name="name" type="text" onKeyUp={updateField} readOnly={form.processing} autoComplete="off" required />
            </fieldset>
            <fieldset>
                <label htmlFor="referral_email">Email</label>
                <input id="referral_email" name="email" type="email" onKeyUp={updateField} readOnly={form.processing} autoComplete="off" required />
            </fieldset>
            <fieldset>
                <button>Send</button>
            </fieldset>
        </form>
        {referrals.length > 0 && <><h3>Added</h3>
        <table>
            <thead>
              	<tr>
	                <th>Date</th>
            ...
            <tbody>
              	{referrals.map((referral,index) => {
              		...
              		return <tr key={index}>
		                ...
		                <td>{referral.status === 'accepted' ? configuration.instagram_coins : 0}</td>
	              	</tr>  
              	})}  
            </tbody>
        </table></>}
