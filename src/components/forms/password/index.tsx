...
const Password: React.FC<Props> = ({ user,userProfile,setNotification }) => {
    ...
    const [data, setData] = useState({...user,player:user.player ? {grade_id:user.player.grade_id,education_id:user.player.education_id,instagram:user.player.instagram,country_iso:user.player.country_iso,state_iso:user.player.state_iso,city_id:user.player.city_id} : {}});
        ...
        if(valid){
            userProfile({...data,_method:'PUT'},(res) => {
                if(!res.errors){
                    ...
                    setNotification({display:true,error:false,content:'Password updated!',timed:true});

                    if(data.id){
                        setForm({data:{...res.data,password:null,password_confirmation:null},...form,processing:false});
                    }
                }else{
	...
	return (<form id="userForm" className={s.form} onSubmit={submit} autoComplete="off" noValidate>
        <fieldset>
